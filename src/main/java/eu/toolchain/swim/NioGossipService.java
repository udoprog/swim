...
public class NioGossipService {
    ...
    public NioGossipService(final EventLoop eventLoop,
            final InetSocketAddress address, final List<Node> seeds,
            final Provider<Boolean> alive) throws IOException {
     ...
     */
    private void handleAck(DatagramChannel c, SocketAddress s, Ack ack)
            throws Exception {
        log.debug("ACK: {}", ack);
    ...
    private void handlePing(final DatagramChannel c, final InetSocketAddress s,
            final Ping ping) throws Exception {
        log.debug("PING: {}", ping);
    ...
    private void sendAck(final DatagramChannel c,
            final InetSocketAddress target, final UUID id, boolean alive)
                    throws Exception {
        send(ACK, c, target, new Ack(id, alive, buildPayloads()),
                AckSerializer.get());
    }

    private void sendPingRequest(final DatagramChannel c,
            final InetSocketAddress target, final InetSocketAddress peer)
                    throws Exception {
        final UUID id = UUID.randomUUID();
        send(PINGREQ, channel, peer, new PingReq(id, target, buildPayloads()),
                PingReqSerializer.get());
        pending.put(id, new PendingPing(System.currentTimeMillis(), target,
                true));
    }

    private void sendPingFromRequest(final DatagramChannel c,
            final InetSocketAddress target, final InetSocketAddress source,
            final UUID pingId) throws Exception {
        final UUID id = UUID.randomUUID();
        send(PING, channel, target, new Ping(id, buildPayloads()),
                PingSerializer.get());
        pending.put(id, new PendingPingReq(System.currentTimeMillis(), target,
                pingId, source));
    }

    private void sendPing(final DatagramChannel c,
            final InetSocketAddress target) throws Exception {
        final UUID id = UUID.randomUUID();
        send(PING, channel, target, new Ping(id, buildPayloads()),
                PingSerializer.get());
        pending.put(id, new PendingPing(System.currentTimeMillis(), target,
    ...
    private <T> void send(byte type, DatagramChannel c, InetSocketAddress to,
            T data, Serializer<T> serializer) throws Exception {
        output.clear();
        output.put(type);
